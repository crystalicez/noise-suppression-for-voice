...
- Refresh the bundled generated RNNoise model data to the model identified by upstream `model_version` (`0a8755f8e2d834eff6a54714ecc7d75f9932e845df35f8b59bc52a7cfe6e8b37`).
...
The generated runtime model files (`src/rnnoise_data.c`, `src/rnnoise_data.h`, and `src/rnnoise_data_little.c`) are refreshed from the model archive named by upstream `model_version`. The archive is accepted only after its SHA-256 matches that exact `model_version` value. A mirror may be used only as a transport fallback if `media.xiph.org` is unreachable; checksum validation remains authoritative.
...
The pull request must exercise the repository's existing GitHub Actions build. The Windows job configures `BUILD_VST_PLUGIN=1`, builds the plugin, runs common tests, and uploads build artifacts. Linux/macOS builds provide additional compile coverage. A passing Windows job using the refreshed generated model data is the primary acceptance criterion for this refresh.
